...
def initialize_movies():
    ...
    rating_data["rating"] = rating_data["rating"].apply(lambda x: float(x)) # unit conversion after sql read might be an issue
    ...
    for genre_ticks_of_particular_movie in genre_ticks_for_each_movie:
        relevant_genres.append(', '.join([genre for genre, tick in zip(
            genre_cols, genre_ticks_of_particular_movie) if tick == 1]))
    ...
    print('movie_data BEFORE BEING SENT BY MOVIE DATAFRAME:')
...
def build_model(rating_data, user_count, movie_count):
    ...
    # helper
    def build_rating_sparse_tensor(rating_data, user_count, movie_count): #so I don't need to have entries for the new user? I can just increase the user count or?
        """
        Args:
...
def user_recommendations(model, movie_data, rating_data, user_id, k=6):
    ...
    df = pd.DataFrame({
        ...
        'genres': movie_data['combined_genres'],
    })
    already_rated_by_user = rating_data[rating_data['user_id']
                               == str(user_id)]["movie_id"].values
    # df = df[df.item_id.apply(lambda movie_id: movie_id not in already_rated_by_user)]
    df = df[~df['id'].isin(already_rated_by_user)] 
