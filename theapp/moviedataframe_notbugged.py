...
print('Retrieving movielens and extracting')
...
rating_data = pd.read_csv('ml-100k/u.data', usecols=[0,1,2], sep='\t',names=['user_id', 'movie_id', 'rating'], encoding='latin-1')
...
item_data = item_data.drop(['release_date','video_release_date', 'imdb_url'], axis=1)
...
def mark_genres(item_data, genres):
  ...
  def get_all_genres(gs):
    ...
    return '-'.join(active)
  item_data['genre'] = [
      get_random_genre(gs) for gs in zip(*[item_data[genre] for genre in genres])]
  item_data['all_genres'] = [
      get_all_genres(gs) for gs in zip(*[item_data[genre] for genre in genres])]

mark_genres(item_data, genre_cols)

# Create one merged DataFrame containing all the movielens data.
# movielens = ratings.merge(item_data, on='movie_id').merge(users, on='user_id')
...
def build_rating_sparse_tensor(ratings_df):
  ...
  return tf.SparseTensor(
      indices=indices,
      values=values,
      dense_shape=[user_count, item_data.shape[0]])
...
def user_recommendations(model, user_id, k=6):
  scores = compute_scores(model.embeddings["user_id"][user_id], model.embeddings["movie_id"])
  score_key = 'similarity'
  df = pd.DataFrame({
      score_key: list(scores),
      'item_id': item_data['movie_id'],
      'titles': item_data['title'],
      'genres': item_data['all_genres'],
  })
  rated_movies = rating_data[rating_data['user_id'] == str(user_id)]["movie_id"].values
...
def movie_neighbors(model, movie_id, k=6):
    scores = compute_scores(model.embeddings["movie_id"][movie_id], model.embeddings["movie_id"])
...
def insert_new_rating(df, user_id, item_id, rating):
  new_rating = (user_id, item_id, rating)
  df.loc[len(rating_data)] = new_rating
